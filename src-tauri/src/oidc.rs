...
/// Subset of the OIDC discovery document we use. aut.hair advertises a custom
/// `machine_info_endpoint` we lean on for the revocation probe.
#[derive(Debug, Clone, Deserialize)]
struct Discovery {
    token_endpoint: String,
    #[serde(default)]
    #[allow(dead_code)] // used by machine_info (Stage 2 P2P revocation probe)
...
/// `/api/machine-info` payload. Deserialized as the wire contract; connect()
/// currently only checks presence (a 200 vs 401), but the identity fields are
/// retained for surfacing in the UI later.
#[derive(Debug, Clone, Deserialize)]
#[allow(dead_code)]
...
/// Mints/caches machine tokens (client_credentials) and runs the machine-info
/// probe. See docs/DIRECT-MODE.md "Auth model".
pub struct Auth {
...
impl Auth {
    ...
    /// A usable Bearer token, or None if the user hasn't onboarded credentials.
    /// Static tokens are returned as-is; client credentials mint/refresh.
    ...
    async fn mint(&self, client_id: &str, client_secret: &str) -> Result<String> {
        ...
        // Refresh a minute early so a token isn't used right at expiry (clock
        // skew / in-flight requests). Floor at 30s for very short-lived tokens.
    ...
    /// Probe `/api/machine-info`. Ok(None) means the token is revoked/expired
    /// (401); Ok(Some(_)) returns the token's machine identity. Used by the
    /// Stage 2 P2P path before dialing an agent directly.
    #[allow(dead_code)]
